...
data <- data[data$nace_division == 10,]
...
means <- c(mean(universal$current_assets, na.rm = TRUE),
...
skewness <- 3 * (means - medians) / standard

names(skewness) <- c("current_assets", "payables", "equity", "share_capital", "total_expenses", "gross_profit_or_loss", "net_profit_or_loss", "current_assets_yoy", "payables_yoy", "equity_yoy", "share_capital_yoy", "total_expenses_yoy", "gross_profit_or_loss_yoy", "net_profit_or_loss_yoy")
...
universal1 <- universal[c(1,2,3,4,5,13,6,7,8,9,10,11,12,18,19,20,21,22,23,24,14,15,16,17)]
...
universal_numeric <- universal1[7:24]
...
reg_data_unclean <- c(universal1[c(6,7,9,10,11,12,14,15,16,17,18,19,20,21,22,24)])
...
Q <- quantile(reg_data_unclean1$total_expenses, probs=c(.25, .75), na.rm = FALSE)
iqr <- IQR(reg_data_unclean1$total_expenses)
reg_data_unclean <- subset(reg_data_unclean1, reg_data_unclean1$total_expenses > (Q[1] - 1.5*iqr) & reg_data_unclean1$total_expenses < (Q[2]+1.5*iqr))
...
Q <- quantile(reg_data_unclean$net_profit_or_loss_yoy, probs=c(.25, .75), na.rm = FALSE)
iqr <- IQR(reg_data_unclean$net_profit_or_loss_yoy)
reg_data_unclean1 <- subset(reg_data_unclean, reg_data_unclean$net_profit_or_loss_yoy > (Q[1] - 1.5*iqr) & reg_data_unclean$net_profit_or_loss_yoy < (Q[2]+1.5*iqr))

Q <- quantile(reg_data_unclean1$share_capital, probs=c(.25, .75), na.rm = FALSE)
iqr <- IQR(reg_data_unclean1$share_capital)
reg_data_unclean <- subset(reg_data_unclean1, reg_data_unclean1$share_capital > (Q[1] - 1.5*iqr) & reg_data_unclean1$share_capital < (Q[2]+1.5*iqr))
...
default_sample <- defaults[sample(nrow(defaults), 259), ]

non_default_sample <- non_defaults[sample(nrow(non_defaults), 518), ]
...
macro_default_sample <- macro_defaults[sample(nrow(macro_defaults), 259), ]

macro_non_default_sample <- macro_non_defaults[sample(nrow(macro_non_defaults), 518), ]
